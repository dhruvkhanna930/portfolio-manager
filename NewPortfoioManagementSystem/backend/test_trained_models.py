...
from dashboard.trained_models import TrainedModelPredictor, compute_trained_model_scores, compute_trained_models_evaluation_matrix
...
def test_evaluation_matrix():
    """Test and print the trained model evaluation matrix."""
    print("=" * 60)
    print("Testing Trained Model Evaluation Matrix")
    print("=" * 60)

    eval_tickers = ['AAPL', 'MSFT', 'GOOGL', 'TSLA']
    matrix = compute_trained_models_evaluation_matrix(eval_tickers, max_samples=50)

    for model_type, results in matrix.items():
        print(f"\n{model_type.upper()} evaluation:")
        for res in results:
            if res.get('available'):
                print(f"  {res['ticker']}: samples={res['samples']}, MAE={res['MAE']}, RMSE={res['RMSE']}, MAPE={res['MAPE']}%, DirectionAccuracy={res['direction_accuracy']}%")
            else:
                print(f"  {res['ticker']}: unavailable ({res.get('message')})")
